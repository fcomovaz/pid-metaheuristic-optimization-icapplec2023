...
function S = stepinfo(sys)
    ...
    yavg_full = conv(ytail, ones (1, 100) / 100);
    yavg = yavg_full(1:length (ttail));

    % yfinal
    [~, settling_ind] = min (abs (abs (yavg - yfinal) - ST * ypeak));
    settling_time = ttail(settling_ind);

    % Calculate Transient Time
    emax = max(abs(y - yfinal));
    transient_time_index = find(abs(y - yfinal) <= ST * emax, 1);
    transient_time = t(transient_time_index);

    S = struct('RiseTime', rise_time, 'SettlingTime', settling_time, ...
        'SettlingMin', settling_min, 'SettlingMax', settling_max, ...
        'Overshoot', overshoot, 'Undershoot', undershoot, ...
        'Peak', abs(ypeak), 'PeakTime', tpeak, ...
        'SteadyStateValue', yfinal, 'TransientTime', transient_time);
end
